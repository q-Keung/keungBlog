...
<div>
  ...
  <div class="home-container" v-if="isloading">
    <div class="search-box">
      <van-search background="#ffffff50" :value="value" show-action="show-action" cancel-class="cancel-class" shape="round" placeholder="请输入搜索关键词"></van-search>
    </div>
      ...
      <van-tabs swipeable="swipeable" animated="animated" color="#1989fa">
        <van-tab title="全部">
          <div class="tab-all" v-for="(item,key) in scrollData" :key="key">
            ...
            <scroll-view class="scroll-body" scroll-x="scroll-x" scroll-with-animation="scroll-with-animation" @scroll="scrollMove">
              <div class="tab-content" v-for="(i,k) in item.items" :key="k">
                <image :src="i.src" mode="scaleToFill" lazy-load="true"></image>
                <p>{{i.title}}</p>
              </div>
            </scroll-view>
          </div>
...
export default {
  ...
  onLoad(options) {
    ...
    this.$http.get('getSwiperData').then(res => {
        console.log(res);
        this.sImgs = res.swiperData;
      })
    this.$http.get('scrollData').then(res => {
        this.scrollData = res.scrollData;
        this.$nextTick(() => {
          setTimeout(() => {
      ...
      const db = wx.cloud.database({ env: 'keung-1f46cf' });
